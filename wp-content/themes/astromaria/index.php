...
<section class="last-articles">
  <div class="last-articles__header">
    <h2 class="last-articles__heading">Последние новости</h2>
    <a class="last-articles__header-link" href="/news/">Смотреть все новости &#8594</a>
  </div>
  <div class="last-articles__content"><?
    foreach ($last_news as $last_news_item) {
      setup_postdata($post); ?>
      <a class="last-articles__item" href="<?=get_permalink($last_news_item->ID)?>">
        <div class="last-articles__image" style="background-image: url('<?=get_the_post_thumbnail_url($last_news_item->ID)?>');"></div>
        <div class="last-articles__item-content">
          <div class="last-articles__date"><?=wp_date('j F Y', strtotime($last_news_item->post_date))?></div>
          <div class="last-articles__name"><?=$last_news_item->post_title?></div>
          <div class="last-articles__description"><?=show_custom_excerpt($last_news_item->post_content)?></div>
        </div>
      </a><?
    }
